...
export default function TestimonialAdmin() {
  ...
  useEffect(() => { async function loadData() { try { const data = await getCmsContent("home", "testimonial"); if (data && Object.keys(data).length > 0) { const flatData = data.desktop || data; setContent({ ...defaultContent, ...flatData }) } } catch (error) { console.error("Failed to load:", error) } finally { setLoading(false) } } loadData() }, [])

  const handleChange = (key: string, value: any) => { setContent(prev => ({ ...prev, [key]: value })) }

  const handleSave = async () => { setSaving(true); try { await upsertCmsContent("home", "testimonial", content); trackSave('testimonial', `Saved testimonial section content`, { device: 'all', testimonialCount: content.testimonials.length }); toast.success("Testimonial updated successfully") } catch (error) { console.error("Failed to save:", error); toast.error("Failed to save changes") } finally { setSaving(false) } }

  const handleTestimonialChange = (index: number, field: string, value: string) => {
    const newItems = [...content.testimonials]; newItems[index] = { ...newItems[index], [field]: value } as any; handleChange( 'testimonials', newItems)
  }
  const addTestimonial = () => { handleChange( 'testimonials', [...content.testimonials, { name: "", role: "", image: "", text: "" }]); trackCreate('testimonial', `Added new testimonial`, {  testimonialCount: content.testimonials.length + 1 }) }
  const removeTestimonial = (index: number) => { handleChange( 'testimonials', content.testimonials.filter((_: any, i: number) => i !== index)); trackDelete('testimonial', `Removed testimonial ${index + 1}`, {  testimonialIndex: index }) }

  const handleImageUpload = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return
    setUploadState({ ...uploadState, [index]: true })
    try { const formData = new FormData(); formData.append("file", file); const { success, finalUrl } = await uploadFile(formData); if (success && finalUrl) { handleTestimonialChange( index, "image", finalUrl); trackUpload('testimonial', `Testimonial ${index + 1} image upload`, {  testimonialIndex: index }); toast.success("Image uploaded successfully") } else { toast.error("Failed to upload image") } } catch (error) { console.error("Upload failed:", error); toast.error("Failed to upload image") } finally { setUploadState({ ...uploadState, [index]: false }) }
  }
  ...
  const renderForm = () => {
    ...
    return (
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 mt-4">
        <div>
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2"><Label>Pill Text</Label><Input value={d.pillText} onChange={e => handleChange( 'pillText', e.target.value)} /></div>
              <div className="space-y-2"><Label>Heading</Label><Textarea value={d.heading} onChange={e => handleChange( 'heading', e.target.value)} rows={2} /></div>
              ...
              <div className="space-y-2"><Label>Description</Label><Textarea value={d.description} onChange={e => handleChange( 'description', e.target.value)} rows={4} /></div>
        ...
        <div>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between"><CardTitle>Testimonials List</CardTitle><Button onClick={() => addTestimonial()} variant="outline" size="sm"><Plus className="mr-2 h-4 w-4" /> Add</Button></CardHeader>
            <CardContent className="space-y-6">
              {d.testimonials.map((item: any, index: number) => (
                <div key={index} className="relative space-y-4 rounded-lg border bg-gray-50 p-4">
                  <Button variant="ghost" size="icon" className="absolute top-2 right-2 text-red-500 hover:bg-red-50 hover:text-red-700" onClick={() => removeTestimonial( index)}><Trash2 className="h-4 w-4" /></Button>
                  ...
                  <div className="space-y-2"><Label>Role</Label><Input value={item.role} onChange={(e) => handleTestimonialChange( index, "role", e.target.value)} /></div>
                  <div className="space-y-2"><Label>Feedback</Label><Textarea value={item.text} onChange={(e) => handleTestimonialChange( index, "text", e.target.value)} rows={3} /></div>
                </div>
              ))}
            </CardContent>
          </Card>
