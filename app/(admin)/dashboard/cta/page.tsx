...
export default function CTACmsPage() {
  ...
  useEffect(() => { async function fetchInitial() { const data = await getCmsContent('home', 'cta'); if (data) { const flatData = data.desktop || data; setContent({ ...defaultContent, ...flatData }) }; setIsLoading(false) } fetchInitial() }, [])
  const handleChange = (key: string, value: any) => { setContent(prev => ({ ...prev, [key]: value })) }
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, ) => { const file = e.target.files?.[0]; if (!file) return; setIsUploading(true); try { const formData = new FormData(); formData.append('file', file); const { success, finalUrl } = await uploadFile(formData); if (success && finalUrl) { handleChange( 'bgImage', finalUrl); trackUpload('cta', `CTA background image upload`); toast.success("Image uploaded") } } catch (err) { toast.error("Upload failed") } finally { setIsUploading(false) } }
  const handleSave = async () => { setIsSaving(true); const { success } = await upsertCmsContent('home', 'cta', content); if (success) { trackSave('cta', `Saved CTA section content`); toast.success("Saved successfully") } else toast.error("Failed to save"); setIsSaving(false) }
      ...
      <div className="grid gap-6 md:grid-cols-2 mt-4">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2"><Label>Heading (Use \n for breaks)</Label><Textarea rows={4} value={d.heading} onChange={e => handleChange( 'heading', e.target.value)} /></div>
