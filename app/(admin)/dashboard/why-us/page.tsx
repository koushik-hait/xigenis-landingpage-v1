...
export default function WhyChooseUsCmsPage() {
  ...
  useEffect(() => { async function fetchInitial() { const data = await getCmsContent('home', 'why-us'); if (data) { const flatData = data.desktop || data; setContent({ ...defaultContent, ...flatData }) }; setIsLoading(false) } fetchInitial() }, [])
  ...
  const handleFeatureChange = (index: number, key: string, value: string) => {
    const newFeatures = [...content.features]; newFeatures[index] = { ...newFeatures[index], [key]: value } as any; handleChange( 'features', newFeatures)
  }
  const handleStatChange = (index: number, key: string, value: string) => {
    const newStats = [...content.footerStats]; newStats[index] = { ...newStats[index], [key]: value } as any; handleChange( 'footerStats', newStats)
  }
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    {d.footerStats.map((stat, i) => (
                        <div key={i} className="space-y-2">
                           <Label className="font-bold">Stat {i+1} Value</Label><Input value={stat.value} onChange={e => handleStatChange( i, 'value', e.target.value)} />
                           <Label className="font-bold">Stat {i+1} Label Text</Label><Input value={stat.label} onChange={e => handleStatChange( i, 'label', e.target.value)} />
                        </div>
                    ))}
                </div>
            </CardContent>
